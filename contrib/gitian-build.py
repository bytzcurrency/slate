...
# Copyright (c) 2019 The Slate developers
...
def setup():
    ...
    if not os.path.isdir('gitian.sigs'):
        subprocess.check_call(['git', 'clone', 'https://github.com/gitianuser/gitian.sigs-slate.git', 'gitian.sigs'])
    if not os.path.isdir('slate-detached-sigs'):
        subprocess.check_call(['git', 'clone', 'https://github.com/gitianuser/slate-detached-sigs.git'])
    if not os.path.isdir('gitian-builder'):
        subprocess.check_call(['git', 'clone', 'https://github.com/devrandom/gitian-builder.git'])
    if not os.path.isdir('slate'):
        subprocess.check_call(['git', 'clone', 'https://github.com/slatecurrency/slate.git'])
...
def build():
    ...
    os.makedirs('slate-binaries/' + args.version, exist_ok=True)
    print('\nBuilding Dependencies\n')
    os.chdir('gitian-builder')
    os.makedirs('inputs', exist_ok=True)

    subprocess.check_call(['wget', '-O', 'inputs/osslsigncode-1.7.1.tar.gz', '-N', '-P', 'inputs', 'https://github.com/gitianuser/osslsigncode/releases/download/v1.7.1/osslsigncode-1.7.1.tar.gz'])
    subprocess.check_call(['wget', '-O', 'inputs/osslsigncode-Backports-to-1.7.1.patch', '-N', '-P', 'inputs', 'https://github.com/gitianuser/osslsigncode/releases/download/v1.7.1/osslsigncode-Backports-to-1.7.1.patch'])
    ...
    subprocess.check_call(['make', '-C', '../slate/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def sign():
    ...
    if args.windows:
        ...
        subprocess.check_call('mv build/out/slate-*win32-setup.exe ../slate-binaries/'+args.version, shell=True)
...
def verify():
    ...
    if args.linux:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-linux', '../slate/contrib/gitian-descriptors/gitian-linux.yml'])
        print('\nVerifying v'+args.version+' Linux\n')
        subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-linux', '../slate/contrib/gitian-descriptors/gitian-linux.yml'])

    if args.windows:
        print('\nVerifying v'+args.version+' Windows\n')
        subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-win-unsigned', '../slate/contrib/gitian-descriptors/gitian-win.yml'])
        if args.sign:
            print('\nVerifying v'+args.version+' Signed Windows\n')
            subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-win-signed', '../slate/contrib/gitian-descriptors/gitian-win-signer.yml'])

    if args.macos:
        print('\nVerifying v'+args.version+' MacOS\n')
        subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-osx-unsigned', '../slate/contrib/gitian-descriptors/gitian-osx.yml'])
        if args.sign:
            print('\nVerifying v'+args.version+' Signed MacOS\n')
            subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-osx-signed', '../slate/contrib/gitian-descriptors/gitian-osx-signer.yml'])
...
def main():
    ...
    if args.pull:
        ...
        os.chdir('../gitian-builder/inputs/slate')
        subprocess.check_call(['git', 'fetch', args.url, 'refs/pull/'+args.version+'/merge'])
