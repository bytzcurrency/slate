...
SLATE Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
zSLX Staking is here! zSLX staking will be activated on the morning of the 8th of May 2018. With the release of zSLX staking, there are effectively 2 versions of zSLX, zSLX minted on the 3.0.6 SLATE wallet or lower, and zSLX minted on SLATE wallet version or higher. New features in this release will require the use of zSLX v2, zSLX minted on this wallet release 3.1.0 or later. If you currently hold zSLX v1 and wish to take advantage of zSLX staking and deterministic zSLX, you will need to spend the zSLX v1 to yourself and remint zSLX v2.
Note: To find your zSLX version, click the privacy tab, then the zSLX Control button then expand the arrows next to the desired denomination.


### Deterministic zSLX Seed Keys

zSLX is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zSLX outside of the wallet that the zSLX had been minted on. zSLX can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zSLX minting

zSLX minting now only requires 1 further mint (down from 2) to mature. zSLX mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zSLX Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zSLX denomination has been spent or not.



SLX/zSLX Staking and Masternode Rewards
--------------

### SLX, zSLX and Masternode Payment Schedule

To encourage the use of zSLX and increase the SLATE zerocoin anonymity set, the SLATE payment schedule has been changed to the following:

If a user staking zSLX wins the reward for their block, the following zSLX reward will be: 
- 3 zSLX (3 x 1 denominations) rewarded to the staker, 2 SLX rewarded to the masternode owner and 1 SLX available for the budget. This is a total block reward of 6 SLX, up from 5.

If a user staking SLX wins the reward, the following amounts will be rewarded: 
- 2 SLX to the SLX staker, 3 SLX to the Masternode owner and 1 SLX available for the budget. This is a total block reward of 6 SLX, up from 5.


### Return change to sender when minting zSLX

Previously, zSLX minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most SLX and return the change from a zSLX mint to that address.
...
The visual layout of the SLATE Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zSLX minting feature. This is enabled by default and the enablezeromint=0 setting in the slate.conf file will overwrite the GUI option.
-	The percentage of autominted zSLX can now be set from 1 to 100, changed from 10 – 100.
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular SLX.
...
### Summary
 ...
 The SLATE wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the slate.conf file, found in the SLATE data directory.
- backuppath = <directory / full path>
- zslxbackuppath = <directory / full path>
...
* If zSLX backup, auto generated name is `wallet-autozslxbackup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
SLATE Daemon & Client (RPC Changes)
--------------

### RPC Ban Management

The SLATE client peer bans now have additional RPC commands to manage peers. Peers can be banned and unbanned at will without the need to restart the wallet client. No changes have been made to the conditions resulting in automatic peer bans. New RPC commands: `setban`, `listbanned`, `clearbanned`, and `disconnectnode`
...
This allows running slated without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the SLATE support group, which often times sees users say "I didn't spend that zSLX". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the SLATE masternodes, the monthly budget available to be utilised has been increased to 42,000 SLX / month. This SLX only has the opportunity to be raised once per month (paid to winning proposals) with any unused SLX not created by the blockchain.

### Budget Finalization Fee

The SLATE finalization fee for successful proposals has now been reduced, this fee is now 5 SLX down from 50 SLX. The total fee outlay for a successful proposal is now a total of 55 SLX.
...
-	Enforced v1 zSLX spends to require a security level of 100
-	Updates to zSLX spends to avoid segfaults
-	Updates to configuration will now reflect on the privacy tab
-	Fixed a  bug that would not start masternodes from the SLATE-Qt masternodes tab
-	Updated SLATE-Qt tooltips
...
Several critical security flaws in the zerocoin protocol and SLATE's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zSLX Control dialog in the QT wallet, a user is able to see zSLX marked as version 1 or 2.

zPoS (zSLX staking)
--------------

Once a zSLX has over 200 confirmations it becomes available to stake. Staking zSLX will consume the exact zerocoin that is staked and replace it with a freshly minted zerocoin of the same denomination as well as a reward of three 1 denomination zSLX. So for example if a 1,000 zSLX denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination zSLXs.

Secure Spending
--------------

Version 1 zerocoins, as implemented by [Miers et. al](http://zerocoin.org/media/pdf/ZerocoinOakland.pdf), allow for something we describe as *serial trolling*. Spending zerocoins requires that the spender reveal their serial number associated with the zerocoin, and in turn that serial number is used to check for double spending. There is a fringe situation (which is very unlikely to happen within SLATE's zerocoin implementation due to delayed coin accumulation) where the spender sends the spending transaction, but the transaction does not immediately make it into the blockchain and remains in the mempool for a long enough duration that a *troll* has enough time to see the spender's serial number, mint a new zerocoin with the same serial number, and spend the new zerocoin before the original spender's transaction becomes confirmed. If the timing of this fringe situation worked, then the original spender's coin would be seen as invalid because the troll was able to have the serial recorded into the blockchain first, thus making the original spender's serial appear as a double spend.
...
Zerocoins, or zSLX, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zSLX that can be recalculated at any time using the seed. Deterministic zSLX allows for users to backup all of their future zSLX by simply recording their seed and keeping it in a safe place (similar to backing up a private key for SLX). The zSLX seed needs to remain in the wallet in order to spend the zSLX after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zSLX data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.

Zerocoin Modulus
--------------

SLATE's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which SLATE used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to SLATE using the bug bounty program. The modulus is now correctly imported and SLATE's accumulators have been changed to use the new proper modulus.
...
### Core Features
 - #582 `cb1632520` [Core] zSLX v2: zPoS dzSLX ezSLX external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zSLX balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zSLX status output (Mrs-X)
 ...
 - #467 `7099831bd` [Qt] Clear PrivacyDialog "zSlx Selected" labels after sending. (presstab)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault slate-cli getinfo while loading block index (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zSLX spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zSLX. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zSLX. (presstab)
