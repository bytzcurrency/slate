SLATE Core version 3.0.4 is now available from:
...
SLATE Core v3.0.4 is a mandatory update for all users. This release contains various updates/fixes pertaining to the zSLX protocol, supply tracking, block transmission and relaying, as well as usability and quality-of-life updates to the GUI.
...
Refactoring of zSlx Spend Validation Code
---------------------
zSlx spend validation was too rigid and did not give enough slack for reorganizations. Many staking wallets were unable to reorganize back to the correct blockchain when they had an orphan stake which contained a zSlx spend. zSlx double spending validation has been refactored to properly account for reorganization.

Money Supply Calculation Fix
---------------------
Coin supply incorrectly was counting spent zSlx as newly minted coins that are added to the coin supply, thus resulting in innacurate coin supply data.
...
---------------------
The stake miner code now filters out zSlx double spends that were on rare occasions being slipped into blocks (and being rejected by peers when broadcast to the network).
...
More Extensive Display of zSlx Confirmation/Maturity Status
---------------------
Within the Privacy Dialog of the QT wallet, the _Zerocoin Stats_ section now displays both the confirmation and maturity status of minted zSlx's. Previously this was only able to be viewed within the zSlx Control dialog. The main Overview tab of the QT wallet now splits zSlx balance into subcategories: _Unconfirmed_, _Immature_, and _Mature_.
...
- #294 `27c0943` Add additional checks for txid for zslx spend. (presstab)
- #301 `b8392cd` Refactor zSlx tx counting code. Add a final check in ConnectBlock() (presstab)
- #306 `77dd55c` [Core] Don't send not-validated blocks (Mrs-X)
- #312 `5d79bea` [Main] Update last checkpoint data (Fuzzbawls)
- #325 `7d98ebe` Reindex zSlx blocks and correct stats. (presstab)
- #327 `aa1235a` [Main] Don't limit zSLX spends from getting into the mempool (Fuzzbawls)
...
### GUI
- #309 `f560ffc` [UI] Better error message when too much inputs are used for spending zSLX (Mrs-X)
...
- #314 `f1c830a` Fix issue causing crash when slated --help was invoked (Jon Spock)
- #326 `8b6a13e` Combine 2 LogPrintf statement to reduce debug.log clutter (Jon Spock)
- #328 `a6c18c8` [Main] SLATE not responding on user quitting app (Aaron Langford)
