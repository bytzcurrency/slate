SLATE Core version 3.0.2 is now available from:
...
SLATE Core v3.0.2 is a recommended, semi-mandatory update for all users. This release contains transaction creation bug fixes for zSLX spends, automint calculation adjustments, and other various updates/fixes.

zSLX spending requires this update.
...
Auto Wallet Backup
---------------------
In addition to the automatic wallet backup that is done at each start of the client, a new automatic backup function has been added that will, by default, create a backup of the wallet file during each zSLX mint operation (zSLX spends which re-mint their change are also included in this). This functionality is controlled by the `-backupzslx` command-line option, which defaults to `1` (enabled, auto-backup).

Users that wish to prevent this behavior (not recommended) can pass `-backupzslx=0` at the command-line when starting the client, or add `backupzslx=0` to their `slate.conf` file.

zSLX Automint Calculations
---------------------
A bug in the automint calculations was made apparent on mainnet when block times exceeded expectations, resulting in zSLX mint transactions that were in an unconfirmed state to still be treated as if they had never been minted. This caused automint to effectively mint more than what was intended.

zSLX Spending Fix
---------------------
The size of zSLX spend transactions is knowingly larger than normal transactions, and while this was expected, a much stricter check against the scriptsig size is used for mainnet, causing the transactions to be rejected by the mempool, and thus not being packaged into any blocks.

zSLX Transaction Recovery
---------------------
Due to the aforementioned issue with zSLX spending, users may find that their attempted spends are now conflicted and zSLX balances are not represented as expected. "Recovery" of these transactions can be done using the following methods:
...
RPC Changes
---------------------
The `bip38decrypt` command has had it's parameter order changed to be more consistent with it's counterpart. The command now expects the SLATE address as it's first parameter and the passphrase as it's second parameter.
...
- #292 `feadab4` Additional checks for double spending of zSlx serials. (presstab)

### Wallet
- #271 `5e9a086` [Wallet] Remove unused member wallet in UnlockContext inner class (Jon Spock)
- #279 `e734010` Add -backupzslx startup flag. (presstab)
- #280 `fdc182d` [Wallet] Fix zSlx spending errors. (presstab)
- #282 `310f216` [Wallet] Count pending zSlx balance for automint. (presstab)
...
- #291 `cb314e6` [Qt] zSlx control quantity/amount fixes (rejectedpromise)
