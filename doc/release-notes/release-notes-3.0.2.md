...
PIVX Core v3.0.2 is a recommended, semi-mandatory update for all users. This release contains transaction creation bug fixes for zSLX spends, automint calculation adjustments, and other various updates/fixes.
...
Users that wish to prevent this behavior (not recommended) can pass `-backupzslx=0` at the command-line when starting the client, or add `backupzslx=0` to their `pivx.conf` file.
