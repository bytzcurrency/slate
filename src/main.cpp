...
//              FormatMoney(nFees), FormatMoney(pindex->nMint), FormatMoney(nAmountZerocoinSpent));
    ...
    if (!IsBlockValueValid(block, nExpectedMint, pindex->nMint)) {
        return state.DoS(100, error("ConnectBlock() : reward pays too much or not enough (actual=%s vs limit=%s)",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    ...
    if (CZSlxStake* zSLX = dynamic_cast<CZSlxStake*>(stake)) {
        ...
        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zSLX stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
