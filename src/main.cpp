...
#if defined(NDEBUG)
#error "SLATE cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uslx) are considered zero fee (for relaying and mining)
...
{
    //Check to see if the zSLX is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zSLX spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zSLX without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zSLX spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zSLX spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zSLX spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zslx");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zSLX mints are not already known
            if (tx.IsZerocoinMint()) {
                for (auto& out : tx.vout) {
                    if (!out.IsZerocoinMint())
                        continue;
...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else if (nHeight > 310 && nHeight <= 95350) {     // 310 + 95040; 2 months and 5 days, until July 1, 2018
        nSubsidy = 0 * COIN;
    } else if (nHeight > 95350 && nHeight <= 620950) {  // 95350 + 525600; 365 days
        nSubsidy = 271 * COIN;
    } else if (nHeight > 620950) { // To be tuned
        nSubsidy = 271 * COIN;
    } else {
        nSubsidy = 0 * COIN;
    }
...
CAmount GetSeeSaw(const CAmount& blockValue, int nMasternodeCount, int nHeight)
{
    ...
    int64_t mNodeCoins = nMasternodeCount * 35000 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZSLXStake)
{
    ...
    if (nHeight <= Params().LAST_POW_BLOCK()) {
        ...
        if (isZSLXStake)
            ret = blockValue > 241 * COIN ? 244 * COIN : 0;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from SLATE
...
void RecalculateZSLXSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zSLX supply
...
bool UpdateZSLXSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
    ...
    if (pindex->pprev && pindex->pprev->GetBlockHeader().nVersion >= BLOCKHEADER_ZEROCOIN_VERSION) {
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateSLXSupply(Params().Zerocoin_StartHeight());
    }

    //Track zSLX money supply in the block index
    if (!UpdateZSLXSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zSLX supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zSlxSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert SLX to zSLX
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zSLX spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zSLX serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zSLX stake block is less than allowed start height", __func__);

    if (CZSlxStake* zSLX = dynamic_cast<CZSlxStake*>(stake)) {
        CBlockIndex* pindexFrom = zSLX->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zSLX stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zSLX stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zSLX->GetValue()));
        if (nChecksum200 != zSLX->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zSLX->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZSLX() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zSLX fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zSLX mints and %d zSLX spends\n", __func__, nMints, nSpends);
