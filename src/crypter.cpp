...
{
    {
        ...
        if (CWalletDB(pwalletMain->strWalletFile).ReadCurrentSeedHash(hashSeed)) {
            ...
            if (!GetDeterministicSeed(hashSeed, nSeed)) {
                return error("Failed to read zSLX seed from DB. Wallet is probably corrupt.");
            }
            pwalletMain->zwalletMain->SetMasterSeed(nSeed, false);
        } else {
            // First time this wallet has been unlocked with dzSLX
    ...
    if(IsCrypted()) {
        if (!IsLocked()) { //if we have password
            ...
            if (EncryptSecret(vMasterKey, kmSeed, hashSeed, vchSeedSecret)) {
                ...
                if (db.WriteZSLXSeed(hashSeed, vchSeedSecret)) {
                    return true;
                }
            }
            strErr = "encrypt seed";
        ...
        if (db.WriteZSLXSeed(hashSeed, ToByteVector(seed))) {
            return true;
        }
        strErr = "save zslxseed to wallet";
    }
                //the use case for this is no password set seed, mint dzSLX,
...
bool CCryptoKeyStore::GetDeterministicSeed(const uint256& hashSeed, uint256& seedOut)
{
    ...
    if (IsCrypted()) {
        if(!IsLocked()) { //if we have password
            ...
            //read encrypted seed
            if (db.ReadZSLXSeed(hashSeed, vchCryptedSeed)) {
        ...
        // wallet not crypted
        if (db.ReadZSLXSeed(hashSeed, vchSeed)) {
