...
CzSLXWallet::CzSLXWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zslx seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSLXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSLXSeed_deprecated()) {
                    LogPrintf("%s: Updated zSLX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zslx seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zslx wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSLXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSLXCount(nCountLastUsed);
    else if (!walletdb.ReadZSLXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSLXWallet::Lock()
{
    seedMaster = 0;
}

void CzSLXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSLXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSLX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSLXWallet::LoadMintPoolFromDB()
{
...
void CzSLXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSLXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSLXWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zslxTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSLXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSLX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zslxTracker which also adds to database
    pwalletMain->zslxTracker->Add(dMint, true);
...
void CzSLXWallet::SeedToZSLX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSLXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSLXWallet::GenerateDeterministicZSLX(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSLXWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSLXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
