...
void FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, bool fProofOfStake, bool fZSLXStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    ...
    if (IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS) && budget.IsBudgetPaymentBlock(pindexPrev->nHeight + 1)) {
        ...
        masternodePayments.FillBlockPayee(txNew, nFees, fProofOfStake, fZSLXStake);
    }
}
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake, bool fZSLXStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    if (!pindexPrev) return;
    ...
    CAmount masternodePayment = GetMasternodePayment(pindexPrev->nHeight, blockValue, 0, fZSLXStake);
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    CAmount requiredMasternodePayment = GetMasternodePayment(nBlockHeight-1, nReward, nMasternode_Drift_Count, txNew.IsZerocoinSpend());
