...
bool CheckBlockSignature(const CBlock& block)
{
    ...
    /** Each block is signed by the private key of the input that is staked. This can be either zSLX or normal UTXO
     *  zSLX: Each zSLX has a keypair associated with it. The serial number is a hash of the public key.
     *  UTXO: The public key that signs must match the public key associated with the first utxo of the coinstake tx.
     */
    CPubKey pubkey;
    bool fzSLXStake = block.vtx[1].IsZerocoinSpend();
    if (fzSLXStake) {
