...
UniValue obfuscation(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "obfuscation <slateaddress> <amount>\n"
            "slateaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "obfuscation <slateaddress> <amount>\n"
            "slateaddress, denominate, or auto (AutoDenominate)"
            "<amount> is a real and will be rounded to the next 0.1" +
            HelpRequiringPassphrase());
...
UniValue getpoolinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"current\": \"addr\",    (string) SLATE address of current masternode\n"
...
UniValue masternode(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  start        - Start masternode configured in slate.conf\n"
...
UniValue masternodeconnect(const UniValue& params, bool fHelp)
{
    if (fHelp || (params.size() != 1))
        throw runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("masternodeconnect", "\"192.168.0.6:37415\"") + HelpExampleRpc("masternodeconnect", "\"192.168.0.6:37415\""));
...
UniValue getmasternodestatus (const UniValue& params, bool fHelp)
{
    if (fHelp || (params.size() != 0))
        throw runtime_error(
            ...
            "  \"addr\": \"xxxx\",        (string) SLATE address for masternode payments\n"
            "  \"status\": \"xxxx\",      (string) Masternode status\n"
...
UniValue getmasternodewinners (const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "    \"winner\": {\n"
            "      \"address\": \"xxxx\",    (string) SLATE MN Address\n"
