...
// SLATE: on startup load spork values from previous session if they exist in the sporkDB
...
int64_t GetSporkValue(int nSporkID)
{
    ...
    if (mapSporksActive.count(nSporkID)) {
        ...
        if (nSporkID == SPORK_2_SWIFTTX_ENABLED) r = SPORK_2_SWIFTTX_DEFAULT;
        if (nSporkID == SPORK_3_SWIFTTX_BLOCK_FILTERING) r = SPORK_3_SWIFTTX_BLOCK_FILTERING_DEFAULT;
        if (nSporkID == SPORK_5_SWIFTTX_MAX_VALUE) r = SPORK_5_SWIFTTX_MAX_VALUE_DEFAULT;
...
int CSporkManager::GetSporkIDByName(std::string strName)
{
    ...
    if (strName == "SPORK_5_SWIFTTX_MAX_VALUE") return SPORK_5_SWIFTTX_MAX_VALUE;
...
{
    if (id == SPORK_2_SWIFTTX_ENABLED) return "SPORK_2_SWIFTTX_ENABLED";
    if (id == SPORK_3_SWIFTTX_BLOCK_FILTERING) return "SPORK_3_SWIFTTX_BLOCK_FILTERING";
    if (id == SPORK_5_SWIFTTX_MAX_VALUE) return "SPORK_5_SWIFTTX_MAX_VALUE";
