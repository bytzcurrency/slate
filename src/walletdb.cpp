...
bool CWalletDB::WriteDeterministicMint(const CDeterministicMint& dMint)
{
    ...
    return Write(make_pair(string("dzslx"), hash), dMint, true);
}

bool CWalletDB::ReadDeterministicMint(const uint256& hashPubcoin, CDeterministicMint& dMint)
{
    return Read(make_pair(string("dzslx"), hashPubcoin), dMint);
}

bool CWalletDB::EraseDeterministicMint(const uint256& hashPubcoin)
{
    return Erase(make_pair(string("dzslx"), hashPubcoin));
}
...
bool CWalletDB::ArchiveDeterministicOrphan(const CDeterministicMint& dMint)
{
    ...
    if (!Erase(make_pair(string("dzslx"), dMint.GetPubcoinHash())))
...
bool CWalletDB::WriteZSLXSeed(const uint256& hashSeed, const vector<unsigned char>& seed)
{
...
bool CWalletDB::EraseZSLXSeed()
{
    uint256 hash;
    if(!ReadCurrentSeedHash(hash)){
        return error("Failed to read a current seed hash");
    }
    if(!WriteZSLXSeed(hash, ToByteVector(base_uint<256>(0) << 256))) {
...
bool CWalletDB::EraseZSLXSeed_deprecated()
{
    return Erase(string("dzs"));
}

bool CWalletDB::ReadZSLXSeed(const uint256& hashSeed, vector<unsigned char>& seed)
{
    return Read(make_pair(string("dzs"), hashSeed), seed);
}

bool CWalletDB::ReadZSLXSeed_deprecated(uint256& seed)
{
    return Read(string("dzs"), seed);
}

bool CWalletDB::WriteZSLXCount(const uint32_t& nCount)
{
    return Write(string("dzc"), nCount);
}

bool CWalletDB::ReadZSLXCount(uint32_t& nCount)
{
...
std::list<CDeterministicMint> CWalletDB::ListDeterministicMints()
{
    ...
    for (;;)
    {
        ...
        CDataStream ssKey(SER_DISK, CLIENT_VERSION);
        if (fFlags == DB_SET_RANGE)
            ssKey << make_pair(string("dzslx"), uint256(0));
        CDataStream ssValue(SER_DISK, CLIENT_VERSION);
        int ret = ReadAtCursor(pcursor, ssKey, ssValue, fFlags);
        fFlags = DB_NEXT;
        if (ret == DB_NOTFOUND)
