...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    if (fZerocoinActive)
        pblock->nVersion = BLOCKHEADER_ZEROCOIN_VERSION;
    else
        pblock->nVersion = BLOCKHEADER_INITIAL_VERSION;
...

    {
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to zslxs that have been in mempool long
                    //and higher priority to zslxs that are large in value
                    ...
                    // zSLX spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zSLX spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                //This zSLX serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
        ...
        if (!fProofOfStake) {
            ...
            //Masternode payment
            FillBlockPayee(txCoinStake, nFees, fProofOfStake, false);   // Not yet adapted to different rate when staking zSLX

            //Sign with updated tx
            pwallet->SignCoinStake(*pwallet, txCoinStake);
        ...
        if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != hashBlockLastAccumulated) {
            //For the period before v2 activation, zSLX will be disabled and previous block's checkpoint is all that will be needed
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("SLATEMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        if (pblock->IsZerocoinStake())
            pwalletMain->zslxTracker->RemovePending(pblock->vtx[1].GetHash());
        return error("SLATEMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("SLATEMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("slate-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (pblock->IsZerocoinStake()) {
                ...
                if (!pwallet->GetZerocoinKey(bnSerial, key)) {
                    LogPrintf("%s: failed to find zSLX with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
                    continue;
                }

                //Sign block with the zSLX key
                if (!SignBlockWithKey(*pblock, key)) {
                    LogPrintf("BitcoinMiner(): Signing new block with zSLX key failed \n");
                    continue;
                }
        ...
        LogPrintf("Running SLATEMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
