...
/**
 * Fees smaller than this (in uslx) are considered zero fee (for transaction creation)
...
bool CWallet::GetMasternodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex)
{
    ...
    AvailableCoins(vPossibleCoins, true, NULL, false, ONLY_350000);
...
{
    return zslxTracker->HasSerial(bnSerial);
}
...
CAmount CWalletTx::GetAnonymizableCredit(bool fUseCache) const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        if (fMasterNode && vout[i].nValue == 350000 * COIN) continue; // do not count MN-like outputs
...
CAmount CWalletTx::GetLockedCredit() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        // Add locked coins; Add masternode collaterals which are handled like locked coins
        if (pwallet->IsLockedCoin(hashTx, i) || (fMasterNode && vout[i].nValue == 350000 * COIN)) {
            nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
        }
...
CAmount CWalletTx::GetLockedWatchOnlyCredit() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        if (fMasterNode && vout[i].nValue == 350000 * COIN) {
            nCredit += pwallet->GetCredit(txout, ISMINE_WATCH_ONLY);
        }
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate)
{
    ...
    bool fCheckZSLX = GetBoolArg("-zapwallettxes", false);
    if (fCheckZSLX)
        zslxTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            //If this is a zapwallettx, need to readd zslx
            if (fCheckZSLX && pindex->nHeight >= Params().Zerocoin_StartHeight()) {
...
CAmount CWallet::GetZerocoinBalance(bool fMatureOnly) const
{
    if (fMatureOnly) {
        ...
        CAmount nBalance = 0;
        vector<CMintMeta> vMints = zslxTracker->GetMints(true);
    ...
    return zslxTracker->GetBalance(false, false);
}
...
{
    return zslxTracker->GetUnconfirmedBalance();
}
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                } else if (nCoinType == ONLY_NOT350000IFMN) {
                    found = !(fMasterNode && pcoin->vout[i].nValue == 350000 * COIN);
                } else if (nCoinType == ONLY_NONDENOMINATED_NOT350000IFMN) {
                    if (IsCollateralAmount(pcoin->vout[i].nValue)) continue; // do not use collateral amounts
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if (found && fMasterNode) found = pcoin->vout[i].nValue != 350000 * COIN; // do not use Hot MN funds
                } else if (nCoinType == ONLY_350000) {
                    found = pcoin->vout[i].nValue == 350000 * COIN;
                } else {
                    found = true;
                }
                if (!found) continue;
                ...
                if (IsLockedCoin((*it).first, i) && nCoinType != ONLY_350000)
                    continue;
...
bool CWallet::SelectStakeCoins(std::list<std::unique_ptr<CStakeInput> >& listInputs, CAmount nTargetAmount)
{
    ...
    if (GetBoolArg("-slxstake", true)) {
        ...
        for (const COutput &out : vCoins) {
            ...
            std::unique_ptr<CSlxStake> input(new CSlxStake());
    ...
    if (GetBoolArg("-zslxstake", true) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() && !IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ...
        for (auto meta : setMints) {
            ...
            if (meta.nHeight < chainActive.Height() - Params().Zerocoin_RequiredStakeDepth()) {
                std::unique_ptr<CZSlxStake> input(new CZSlxStake(meta.denom, meta.hashStake));
                listInputs.emplace_back(std::move(input));
            }
        }
    }
...
bool CWallet::MintableCoins()
{
    ...
    CAmount nZslxBalance = GetZerocoinBalance(false);
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    //if we're doing only denominated, we need to round up to the nearest .1 SLX
    if (coin_type == ONLY_DENOMINATED) {
        // Make outputs by looping through denominations, from large to small
        BOOST_FOREACH (CAmount v, obfuScationDenominations) {
            BOOST_FOREACH (const COutput& out, vCoins) {
                if (out.tx->vout[out.i].nValue == v                                               //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1 * COIN) + 100 //round the amount up to .1 SLX over
                    ) {
                    CTxIn vin = CTxIn(out.tx->GetHash(), out.i);
                    int rounds = GetInputObfuscationRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax)
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        //if(out.tx->vout[out.i].nValue == 350000*COIN) continue;
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            bool fAccepted = false;

            // Function returns as follows:
            //
            // bit 0 - 10000 SLX+1 ( bit on if present )
            // bit 1 - 1000 SLX+1
            // bit 2 - 100 SLX+1
            // bit 3 - 10 SLX+1
            // bit 4 - 1 SLX+1
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& setCoinsRet, CAmount& nValueRet, int nObfuscationRoundsMin, int nObfuscationRoundsMax) const
{
    ...
    vector<COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, false, nObfuscationRoundsMin < 0 ? ONLY_NONDENOMINATED_NOT350000IFMN : ONLY_DENOMINATED);
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (fMasterNode && out.tx->vout[out.i].nValue == 350000 * COIN) continue; //masternode input
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 SLX collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    vector<pair<CScript, CAmount> > vecSend;
    vecSend.push_back(make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 SLX collateral
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    ...
                    } else if (coin_type == ONLY_NOT350000IFMN) {
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 350000 SLX.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT350000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 350000 SLX.");
                    } else {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-slate-address
                    CScript scriptChange;
...
void CWallet::AutoZeromint()
{
    ...
    // zSLX are integers > 0, so we can't mint 10% of 9 SLX
    if (nBalance < 10){
        LogPrint("zero", "CWallet::AutoZeromint(): available balance (%ld) too small for minting zSLX\n", nBalance);
        return;
    }

    // Percentage of zSLX we already have
    ...
    if(dPercentage >= nZeromintPercentage){
        LogPrint("zero", "CWallet::AutoZeromint() @block %ld: percentage of existing zSLX (%lf%%) already >= configured percentage (%d%%). No minting needed...\n",
                  chainActive.Tip()->nHeight, dPercentage, nZeromintPercentage);
        return;
    }

    // zSLX amount needed for the target percentage
    nToMintAmount = ((nZerocoinBalance + nBalance) * nZeromintPercentage / 100);

    // zSLX amount missing from target (must be minted)
    nToMintAmount = (nToMintAmount - nZerocoinBalance) / COIN;

    // Use the biggest denomination smaller than the needed zSLX We'll only mint exact denomination to make minting faster.
    ...
    // If a preferred denomination is used (means nPreferredDenom != 0) do nothing until we have enough SLX to mint this denomination
    ...
    CWalletTx wtx;
        ...
        LogPrintf("CWallet::AutoZeromint() @ block %ld: successfully minted %ld zSLX. Current percentage of zSLX: %lf%%\n",
...
int CMerkleTx::GetTransactionLockSignatures() const
{
    ...
    if (!IsSporkActive(SPORK_2_SWIFTTX_ENABLED)) return -3;
...
bool CWallet::CreateZSLXOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwalletMain->GenerateDeterministicZSLX(denomination, coin, dMint);
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, vector<CDeterministicMint>& vDMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    while (true) {
        ...
        if (!CreateZSLXOutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new zslx output", __func__);
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to Slx
    ...
    // Sign if these are slate outputs - NOTE that zSLX outputs are signed later in SoK
    if (!isZCSpendChange) {
        int nIn = 0;
        for (const std::pair<const CWalletTx*, unsigned int>& coin : setCoins) {
            if (!SignSignature(*this, *coin.first, txNew, nIn++)) {
                         ...
                         CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    receipt.SetStatus(_("Transaction Mint Started"), ZSLX_TXMINT_GENERAL);
    ...
    if (!pubCoinSelected.validate()) {
        receipt.SetStatus(_("The selected mint coin is an invalid coin"), ZSLX_INVALID_COIN);
        return false;
    }
    ...
    if (!GenerateAccumulatorWitness(pubCoinSelected, accumulator, witness, nSecurityLevel, nMintsAdded, strFailReason, pindexCheckpoint)) {
        receipt.SetStatus(_("Try to spend with a higher security level to include more coins"), ZSLX_FAILED_ACCUMULATOR_INITIALIZATION);
    ...
    if (nVersion >= libzerocoin::PrivateCoin::PUBKEY_VERSION) {
        ...
        if (!zerocoinSelected.GetKeyPair(key))
            return error("%s: failed to set zSLX privkey mint version=%d", __func__, nVersion);
    ...
    try {
        ...
        if (!spend.Verify(accumulator)) {
            receipt.SetStatus(_("The new spend coin transaction did not verify"), ZSLX_INVALID_WITNESS);
            //return false;
        ...
        //Add the coin spend into a SLATE transaction
        ...
        try {
            ...
            receipt.SetStatus(_("Failed to deserialize"), ZSLX_BAD_SERIALIZATION);
            return false;
        }
        ...
        if (!newSpendChecking.Verify(accumulator)) {
            receipt.SetStatus(_("The transaction did not verify"), ZSLX_BAD_SERIALIZATION);
            return false;
        }

        if (IsSerialKnown(spend.getCoinSerialNumber())) {
            //Tried to spend an already spent zSLX
            receipt.SetStatus(_("The coin spend has been used"), ZSLX_SPENT_USED_ZSLX);

            uint256 hashSerial = GetSerialHash(spend.getCoinSerialNumber());
            if (!zslxTracker->HasSerialHash(hashSerial))
                return error("%s: serialhash %s not found in tracker", __func__, hashSerial.GetHex());

            CMintMeta meta = zslxTracker->Get(hashSerial);
            meta.isUsed = true;
            if (!zslxTracker->UpdateState(meta))
        ...
        receipt.SetStatus(_("CoinSpend: Accumulator witness does not verify"), ZSLX_INVALID_WITNESS);
        return false;
    }

    receipt.SetStatus(_("Spend Valid"), ZSLX_SPEND_OKAY); // Everything okay
...
bool CWallet::CreateZerocoinSpendTransaction(CAmount nValue, int nSecurityLevel, CWalletTx& wtxNew, CReserveKey& reserveKey, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vSelectedMints, vector<CDeterministicMint>& vNewMints, bool fMintChange,  bool fMinimizeChange, CBitcoinAddress* address)
{
    ...
    int nStatus = ZSLX_TRX_FUNDS_PROBLEMS;
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zSLX"), nStatus);
        return false;
    }

    // Create transaction
    nStatus = ZSLX_TRX_CREATE;
    ...
    const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zSLX transaction
    vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        setMints = zslxTracker->ListMints(true, true, true); // need to find mints to spend
        if(setMints.empty()) {
        ...
        // Select the zSLX mints to use in this spend
    ...
    for (const auto& mint : vSelectedMints) {
        if (mint.GetVersion() < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
            if (nSecurityLevel < 100) {
                nStatus = ZSLX_SPEND_V1_SEC_LEVEL;
                receipt.SetStatus(_("Version 1 zSLX require a security level of 100 to successfully spend."), nStatus);
                return false;
            }
        }
    }
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            //add output to slate address to the transaction (the actual primary spend taking place)
            ...
            if (nBytes >= MAX_ZEROCOIN_TX_SIZE) {
                receipt.SetStatus(_("In rare cases, a spend with 7 coins exceeds our maximum allowable transaction size, please retry spend using 6 or less coins"), ZSLX_TX_TOO_LARGE);
                return false;
            }
    ...
    receipt.SetStatus(_("Transaction Created"), ZSLX_SPEND_OKAY); // Everything okay
...
void CWallet::ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored, std::list<CDeterministicMint>& listDMintsRestored)
{
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!zslxTracker->UnArchive(hashPubcoin, false)) {
            LogPrintf("%s : failed to unarchive mint %s\n", __func__, mint.GetValue().GetHex());
        } else {
            zslxTracker->UpdateZerocoinMint(mint);
        }
    ...
    for (CDeterministicMint dMint : listDMints) {
        ...
        if (!zslxTracker->UnArchive(dMint.GetPubcoinHash(), true)) {
            LogPrintf("%s : failed to unarchive deterministic mint %s\n", __func__, dMint.GetPubcoinHash().GetHex());
        } else {
            zslxTracker->Add(dMint, true);
        }
        listDMintsRestored.emplace_back(dMint);
    }
}

string CWallet::GetUniqueWalletBackupName(bool fzslxAuto) const
{
    ...
    return strprintf("wallet%s.dat%s", fzslxAuto ? "-autozslxbackup" : "", DateTimeStrFormat(".%Y-%m-%d-%H-%M", GetTime()));
}

void CWallet::ZSlxBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        strNewBackupName = strprintf("wallet-autozslxbackup-%d.dat", i);
        ...
        if (filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozslxbackup-%d.dat", i + 1);
            ...
            strNewBackupName = strprintf("wallet-autozslxbackup-%d.dat", 0);
            backupPath = backupDir / strNewBackupName;
            break;
        }
    ...
    if(!GetArg("-zslxbackuppath", "").empty()) {
        filesystem::path customPath(GetArg("-zslxbackuppath", ""));
...
bool CWallet::SpendZerocoin(CAmount nAmount, int nSecurityLevel, CWalletTx& wtxNew, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vMintsSelected, bool fMintChange, bool fMinimizeChange, CBitcoinAddress* addressTo)
{
    ...
    int nStatus = ZSLX_SPEND_ERROR;

    if (IsLocked()) {
        receipt.SetStatus("Error: Wallet locked, unable to create transaction!", ZSLX_WALLET_LOCKED);
        return false;
    }
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        nStatus = ZSLX_COMMIT_FAILED;

        //reset all mints
        for (CZerocoinMint mint : vMintsSelected) {
            uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
            zslxTracker->SetPubcoinNotUsed(hashPubcoin);
        ...
        for (CZerocoinSpend spend : receipt.GetSpends()) {
            if (!walletdb.EraseZerocoinSpendSerialEntry(spend.GetSerial())) {
                receipt.SetStatus("Error: It cannot delete coin serial number in wallet", ZSLX_ERASE_SPENDS_FAILED);
            }
        ...
        for (auto& dMint : vNewMints) {
            if (!walletdb.EraseDeterministicMint(dMint.GetPubcoinHash())) {
                receipt.SetStatus("Error: Unable to cannot delete zerocoin mint in wallet", ZSLX_ERASE_NEW_MINTS_FAILED);
            }
        }
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        zslxTracker->SetPubcoinUsed(hashPubcoin, txidSpend);

        CMintMeta metaCheck = zslxTracker->GetMetaFromPubcoin(hashPubcoin);
        if (!metaCheck.isUsed) {
    ...
    receipt.SetStatus("Spend Successful", ZSLX_SPEND_OKAY);  // When we reach this point spending zSLX was successful
...
{
    CMintMeta meta;
    if (!zslxTracker->GetMetaFromStakeHash(hashStake, meta))
...
{
    if (zslxTracker->HasPubcoin(bnValue))
        return true;
...
bool CWallet::UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CZerocoinMint mint;
    if (zslxTracker->HasPubcoinHash(hashValue)) {
        CMintMeta meta = zslxTracker->GetMetaFromPubcoin(hashValue);
        meta.nHeight = nHeight;
        meta.txid = txid;
        return zslxTracker->UpdateState(meta);
    } else {
