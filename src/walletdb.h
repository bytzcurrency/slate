...
public:
    ...
    bool WriteZSLXSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZSLXSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZSLXSeed_deprecated(uint256& seed);
    bool EraseZSLXSeed();
    bool EraseZSLXSeed_deprecated();

    bool WriteZSLXCount(const uint32_t& nCount);
    bool ReadZSLXCount(uint32_t& nCount);
