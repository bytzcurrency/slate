...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::SLX);
    ...
    if (!settings.contains("nAnonymizeSlateAmount"))
        settings.setValue("nAnonymizeSlateAmount", 1000);

    nAnonymizeSlateAmount = settings.value("nAnonymizeSlateAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeSlateAmount"))
        SoftSetArg("-anonymizeslateamount", settings.value("nAnonymizeSlateAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in slate.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSlateAmount:
            return QVariant(nAnonymizeSlateAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSlateAmount:
            nAnonymizeSlateAmount = value.toInt();
            settings.setValue("nAnonymizeSlateAmount", nAnonymizeSlateAmount);
            emit anonymizeSlateAmountChanged(nAnonymizeSlateAmount);
            break;
        case CoinControlFeatures:
