...
#include "ui_zslxcontroldialog.h"
...
std::list<std::string> ZSlxControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZSlxControlDialog::listMints;
...
ZSlxControlDialog::~ZSlxControlDialog()
{
    delete ui;
}

void ZSlxControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSlxControlDialog::updateList()
{
...
void ZSlxControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSlxControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelzSLX_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSlxControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZSlxControlDialog::GetSelectedMints()
{
...
void ZSlxControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
