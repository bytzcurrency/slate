...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered slate address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_SWIFTTX_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 SLX.").arg(GetSporkValue(SPORK_5_SWIFTTX_MAX_VALUE)),
        ...
        if (recipients[0].useSwiftTX && newTx->GetValueOut() > GetSporkValue(SPORK_5_SWIFTTX_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 SLX.").arg(GetSporkValue(SPORK_5_SWIFTTX_MAX_VALUE)),
                CClientUIInterface::MSG_ERROR);
            return TransactionCreationFailed;
        }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal slate:URI (slate:XyZ...?message=example)
            {
...
static void NotifyzSLXReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzSLXReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzSLXReset.connect(boost::bind(NotifyzSLXReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzSLXReset.disconnect(boost::bind(NotifyzSLXReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zslxTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
